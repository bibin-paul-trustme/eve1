...
func processVerifyLogs(t *testing.T) {
	...
	for _, logGzipped := range allLogs {
		...
		verifKey := evolveKey(sk0, keyIter)
...
func TestSecureLogAggregationPerformance(t *testing.T) {
	...
	for _, run := range runs {
		...
		stat, _ := file.Stat()
		if stat.Size() == 0 {
			writer.Write([]string{"Test Run", "Average Secure Time (us)", "Average Non-Secure Time (us)", "Performance Cost (us)", "Performance Overhead (%)"})
		}
