...
There is a symbolic link '/persist/newlog/collect/current.device.log' points to the temporary "keep" device logfile in the /persist/newlog/collect directory. One can use 'tail -F' on this symbolic link file to monitor the output of all the device side (not application side) logs.
...
Once the "upload" gzip log files are uploaded to the cloud, they will be removed. For any log files are still waiting to be uploaded, they are in the '/persist/newlog/devUpload' and '/persist/newlog/appUpload' directories. In the case the network connection to the cloud is good, but the logfile has repeatedly failed to upload, it will be moved out of the 'Upload' directory to '/persist/newlog/failedUpload' directory. EVE developers who have enabled ssh to the device for debugging purposes can look at the log entries in those directories by using "zcat" utility.
...
flowchart TD
    ...
    A[EVE Service Components]
    A -->|
      Agent-specific Log Levels:<br/>agent.*agentname*.debug.loglevel
      OR
      Default Log Level:<br/>debug.default.loglevel
      | LP
    B[Kernel Logs]
    B -->|Log Level:<br/>debug.kernel.loglevel| LP
    C[OS System Components]
    C -->|Log Level:<br/>debug.syslog.loglevel| LP
    LP -->|Filtered by<br/>debug.remote.loglevel| D[Logs Sent to Controller
    /persist/newlog/devUpload
    /persist/newlog/failUpload]
    LP -->|Filtered by<br/>debug.local.loglevel| E[Logs Stored on Device
    /persist/newlog/keepSentQueue]
```

**Note:** As can be seen from the flow diagram, the filtering log levels (`debug.remote.loglevel` and `debug.local.loglevel`) should be set to levels equal to or less verbose than the baseline log levels. Setting them to a more verbose level wouldn't capture additional logs since those logs aren't generated at lower verbosity levels.

There are no nobs for the edge apps' log levels - all logs generated by the edge apps are sent to the controller AND kept on the device.

In addition to the common log levels such as `panic`, `fatal`, `error`, `warning`, `info`, `debug`, and `trace`, we also have `none` and `all` log levels. The `none` log level is used to suppress all logs, while the `all` log level is used to capture all logs.
...
The already uploaded gzip files with app logs are moved to /persist/newlog/keepSentQueue directory or removed in case of dev.log.upload files. This directory and together with 'collect', 'appUpload', 'devUpload' directories form a circular buffer and will be kept up to the quota limit (default is 2 Gbytes and can be changed by user config-item).
